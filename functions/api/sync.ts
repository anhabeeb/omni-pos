...
const jsonResponse = (data: any, status = 200) => {
  return new Response(JSON.stringify(data), {
    ...
    headers: { 
      ...
      'X-OmniPOS-Verification': 'authorized', // Key header to prove this isn't the "Hello World" worker
  ...
  try {
    ...
    if (action === 'WRITE_TEST') {
      try {
        ...
        return jsonResponse({ success: false, error: 'Database bound but unreachable: ' + e.message, hint: 'Run INIT_SCHEMA or check D1 permissions.' });
      }
    }
