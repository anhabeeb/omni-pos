  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-10.5rem)] overflow-hidden relative">
      ...
      <aside className="w-full lg:w-[380px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 flex flex-col shadow-2xl overflow-hidden h-full rounded-3xl">
          ...
          <div className="p-6 bg-gray-50/50 dark:bg-gray-950/30 border-t border-gray-100 dark:border-gray-800 shrink-0"><div className="space-y-2 mb-6"><div className="flex justify-between items-center text-[11px] font-bold text-gray-400 uppercase tracking-widest"><span>Subtotal</span><span className="text-gray-900 dark:text-white font-black">{store?.currency}{totals.subtotal.toFixed(2)}</span></div><div className="relative"><div className="flex justify-between items-center text-[11px] font-bold text-gray-400 uppercase tracking-widest"><span>Discount</span><div className="flex items-center gap-3">{totals.discountAmount > 0 && (<span className="text-red-500 font-black">-{store?.currency}{totals.discountAmount.toFixed(2)}</span>)}<button onClick={(e: React.MouseEvent) => { e.stopPropagation(); setShowDiscountPresets(!showDiscountPresets); }} className={`flex items-center gap-1.5 px-3 py-1 rounded-xl border transition-all ${discountPercent > 0 ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-100'}`}><Percent size={12}/><span className="text-[10px] font-black">{discountPercent}%</span></button></div></div>{showDiscountPresets && (<div className="absolute bottom-full right-0 mb-3 w-64 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-3xl shadow-2xl p-5 z-30"><div className="flex justify-between items-center mb-4"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Presets</p><button onClick={() => { setDiscountPercent(0); setShowDiscountPresets(false); }} className="text-[10px] font-black text-red-500 uppercase hover:underline">Reset</button></div><div className="grid grid-cols-4 gap-2 mb-4">{DISCOUNT_PRESETS.map((p: number) => (<button key={p} onClick={() => { setDiscountPercent(p); setShowDiscountPresets(false); }} className={`py-2.5 rounded-xl text-xs font-black border transition-all ${discountPercent === p ? 'bg-blue-600 text-white' : 'bg-gray-50'}`}>{p}%</button>))}</div><div className="space-y-2"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Custom</p><div className="relative"><input type="number" className="w-full p-3 bg-gray-50 dark:bg-gray-900 border-none rounded-xl text-sm font-black outline-none" value={discountPercent || ''} onChange={(e) => setDiscountPercent(Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))}/><div className="absolute right-4 top-3 text-gray-400 font-black text-sm">%</div></div></div></div>)}</div>{(totals.serviceCharge > 0 || (store?.serviceChargeRate && store.serviceChargeRate > 0)) && (<div className="flex justify-between items-center text-[11px] font-bold text-gray-400 uppercase tracking-widest"><span>Service Charge {store?.serviceChargeRate ? `(${store.serviceChargeRate}%)` : ''}</span><span className="text-gray-900 dark:text-white font-black">{store?.currency}{totals.serviceCharge.toFixed(2)}</span></div>)}<div className="flex justify-between items-center text-[11px] font-bold text-gray-400 uppercase tracking-widest"><span>GST ({store?.taxRate}%)</span><span className="text-gray-900 dark:text-white font-black">{store?.currency}{totals.tax.toFixed(2)}</span></div><div className="flex justify-between items-center pt-4 mt-2 border-t border-gray-200 dark:border-gray-800"><span className="text-xs font-black text-gray-900 dark:text-white uppercase tracking-tighter">Total Payable</span><span className="text-4xl font-black text-blue-600 tracking-tighter">{store?.currency}{totals.total.toFixed(2)}</span></div></div><div className="grid grid-cols-2 gap-3 mb-2"><button onClick={handleSendToKitchen} disabled={cart.length === 0} className={`flex items-center justify-center gap-2 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-30 ${isKOTEnabled ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 hover:bg-orange-200' : 'bg-orange-600 text-white hover:bg-orange-700 shadow-xl shadow-orange-500/20'}`}>{isKOTEnabled ? <><ChefHat size={18}/> KOT</> : <><History size={18}/> Paylater</>}</button><button onClick={handleCheckout} disabled={cart.length === 0} className="flex items-center justify-center gap-2 py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-500/30 hover:bg-blue-700 transition-all active:scale-[0.98] disabled:opacity-30"><DollarSign size={20}/> Pay</button></div></div></aside>
      )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-4">
              <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                  ...
                  <form onSubmit={handleQuickAddCustomer} className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar"><div className="flex gap-4 p-1 bg-gray-100 dark:bg-gray-700 rounded-xl mb-2"><button type="button" onClick={() => setNewCustData({...newCustData, type: 'INDIVIDUAL'})} className={`flex-1 py-2.5 text-xs font-black uppercase rounded-lg ${newCustData.type === 'INDIVIDUAL' ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}>Individual</button><button type="button" onClick={() => setNewCustData({...newCustData, type: 'COMPANY'})} className={`flex-1 py-2.5 text-xs font-black uppercase rounded-lg ${newCustData.type === 'COMPANY' ? 'bg-white shadow text-purple-600' : 'text-gray-500'}`}>Company</button></div>
                      ...
                      <div className="flex gap-4 pt-4 border-t dark:border-gray-700"><button type="button" onClick={() => setIsCustomerModalOpen(false)} className="flex-1 py-4 text-xs font-black uppercase text-gray-400">Discard</button><button type="submit" disabled={isSaving} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase shadow-2xl hover:bg-blue-700 transition-all flex items-center justify-center gap-3">{isSaving ? <Loader2 className="animate-spin" size={18}/> : <CheckCircle size={18}/>} Save Record</button></div>
                  </form>
              </div>
          </div>
      )}
