...
export default function StoreHistory() {
  ...
  const getPaidAmount = (order: Order) => {
      const amount = order.transactions?.reduce((acc, t) => {
          if (t.type === 'PAYMENT') return acc + t.amount;
          if (t.type === 'REVERSAL' || t.type === 'REFUND') return acc - t.amount;
          return acc;
      }, 0) || 0;
  ...
  const handleProcessRefund = async () => {
      ...
      const amountToRefund = Math.round((parseFloat(refundAmount) || 0) * 100) / 100;
      ...
      // Use epsilon (0.0001) for comparison to handle floating point math issues
      if (amountToRefund > (paidAmount + 0.0001)) { 
      ...
      // Check if this refund covers the remaining balance (with epsilon)
      if (refundMode === 'FULL' || Math.abs(paidAmount - amountToRefund) < 0.0001) { 
